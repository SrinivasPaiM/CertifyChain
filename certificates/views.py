...
def generate_certificate_pdf(request, certificate_id):
    ...
    verify_path = reverse('verify_certificate')
    verify_url = request.build_absolute_uri(f"{verify_path}?certificate_id={cert.certificate_id}")
    qr_data = verify_url
    qr.add_data(qr_data)
    qr.make(fit=True)
    ...
    c.drawCentredString(width / 2, 50, "Scan the QR code to open online certificate verification.")
